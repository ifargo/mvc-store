...
class Pagination
{
    ...
    public function get()
    {
        ...
        $html = '<ul class="page_nav d-flex flex-row pagination">';
        # Генерируем ссылки
        ...
        if (!is_null($links)) {
            ...
            # Создаём ссылку "На первую"
                $links = $this->generateHtml(1, '<div class="page_prev d-flex flex-column align-items-center justify-content-center"><i class="fas fa-chevron-left"></i></div>') . $links;

            # Если текущая страница не первая
            if ($this->current_page < $this->amount)
            # Создаём ссылку "На последнюю"
                $links .= $this->generateHtml($this->amount, '<div class="page_next d-flex flex-column align-items-center justify-content-center"><i class="fas fa-chevron-right"></i></div>');
        }
